...
LOGMODULENAMES_JAVA=src/main/java/com/viklauverk/common/log/LogModuleNames.java

logmodulenames:
	@echo "// Copyright (C) 2021-2024 Viklauverk AB (agpl-3.0-or-later)" > $(LOGMODULENAMES_JAVA)
	@echo "// Generated by \"make logmodules\"" >> $(LOGMODULENAMES_JAVA)
	@echo "package com.viklauverk.common.log;" >> $(LOGMODULENAMES_JAVA)
	@echo "public enum LogModuleNames" >> $(LOGMODULENAMES_JAVA)
	@echo "{" >> $(LOGMODULENAMES_JAVA)
	@find src/main/java/com/viklauverk/ -name "*.java" \
			-exec grep -o "LogModule.lookup.*" \{\} \; | cut -f 1 -d ',' | sort -u | grep -v String \
