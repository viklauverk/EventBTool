...
public class Sys
{
    ...
    public String loadMachinesAndContexts(String path) throws Exception
    {
        ...
        // Build symbol tables needed for parsing.
        buildSymbolTablesForContexts();
        buildSymbolTablesForMachines();

        // Now we can actually parse the formulas and figure out the types.
        parseContextFormulas();
        parseMachineFormulas();

        // Now load and parse the checked types generated by Rodin.
//        loadCheckedTypesForContexts();
//        loadCheckedTypesForMachines();
    ...
    private void buildSymbolTablesForContexts() throws Exception
    {
        for (String name : contextNames())
        {
            Context c = getContext(name);
    ...
    private void buildSymbolTablesForMachines() throws Exception
    {
        for (String name : machineNames())
        {
            Machine m = getMachine(name);
            m.buildSymbolTable(rootSymbolTable());
        }
    }

    private void loadCheckedTypesForContexts() throws Exception
    {
        for (String name : contextNames())
        {
            Context c = getContext(name);
    ...
    private void loadCheckedTypesForMachines() throws Exception
    {
        for (String name : machineNames())
        {
            Machine m = getMachine(name);
