...
public class FormulaBuilder extends EvBFormulaBaseVisitor<Formula>
{
    ...
    {
        return FormulaFactory.newAnySymbol(ctx.sym.getText());
    }
    ...
    {
        return FormulaFactory.newNumberSymbol(ctx.sym.getText());
    }
    ...
    {
        return FormulaFactory.newPredicateSymbol(ctx.sym.getText());
    }

    @Override
    public Formula visitExpressionSymbol(EvBFormulaParser.ExpressionSymbolContext ctx)
    {
        return FormulaFactory.newExpressionSymbol(ctx.sym.getText());
    }

    @Override
    public Formula visitSetSymbol(EvBFormulaParser.SetSymbolContext ctx)
    {
        return FormulaFactory.newSetSymbol(ctx.sym.getText());
    }
    ...
    public Formula visitExpressionVariable(EvBFormulaParser.ExpressionVariableContext ctx)
    {
        if (ctx.PRIM() != null)
        {
            return FormulaFactory.newVariablePrimSymbol(ctx.variable.getText());
        }
        else
        {
            return FormulaFactory.newVariableSymbol(ctx.variable.getText());
        }
    }
    ...
    {
        return FormulaFactory.newVariableNonFreeSymbol(ctx.variable.getText());
    }
    ...
    {
        return FormulaFactory.newConstantSymbol(ctx.constant.getText());
    }
    ...
    public Formula visitVariableFunctionApplication(EvBFormulaParser.VariableFunctionApplicationContext ctx)
    {
        ...
        if (ctx.PRIM() != null)
        {
            var = FormulaFactory.newVariablePrimSymbol(ctx.variable.getText());
        }
        else
        {
            var = FormulaFactory.newVariableSymbol(ctx.variable.getText());
        }
        if (ctx.INV() != null)
        {
            return FormulaFactory.newFunctionInvertedApplication(var, this.visit(ctx.inner));
        }
        else
        {
            return FormulaFactory.newFunctionApplication(var, this.visit(ctx.inner));
        }
    }
    ...
    {
        Formula constant = FormulaFactory.newConstantSymbol(ctx.constant.getText());
        return FormulaFactory.newFunctionApplication(constant, this.visit(ctx.inner));
    }

    @Override
    public Formula visitGenericFunctionApplication(EvBFormulaParser.GenericFunctionApplicationContext ctx)
    {
        return FormulaFactory.newFunctionApplication(this.visit(ctx.function), this.visit(ctx.inner));
    }

    @Override
    public Formula visitExpressionTRUE(EvBFormulaParser.ExpressionTRUEContext ctx)
    {
        return FormulaFactory.newConstantSymbol("TRUE");
    }

    @Override
    public Formula visitExpressionFALSE(EvBFormulaParser.ExpressionFALSEContext ctx)
    {
        return FormulaFactory.newConstantSymbol("FALSE");
    }
    ...
    public Formula visitListOfNonFreeVariables(EvBFormulaParser.ListOfNonFreeVariablesContext ctx)
    {
        ...
        for (org.antlr.v4.runtime.tree.TerminalNode sec : ctx.SYMBOL())
        {
            ...
            elements.add(FormulaFactory.newVariableNonFreeSymbol(sec.getText()));
        }
    ...
    public Formula visitBecomeEQFuncApp(EvBFormulaParser.BecomeEQFuncAppContext ctx)
    {
        ...
        return FormulaFactory.newBecomeEQFuncApp(var, left, right);
    }
    ...
    public Formula visitBecomeIN(EvBFormulaParser.BecomeINContext ctx)
    {
        ...
        return FormulaFactory.newBecomeIN(var, set);
    }
    ...
    public Formula visitBecomeSUCH(EvBFormulaParser.BecomeSUCHContext ctx)
    {
        Formula var = FormulaFactory.newVariableSymbol(ctx.variable.getText());
        Formula set = this.visit(ctx.inner);
        return FormulaFactory.newBecomeSUCH(var, set);
    }
    ...
    {
        return FormulaFactory.newNumber(ctx.NUMBER().getText());
    }

    @Override
    public Formula visitConjunction(EvBFormulaParser.ConjunctionContext ctx)
    {
        Formula f = FormulaFactory.newConjunction(this.visit(ctx.left), this.visit(ctx.right));
    ...
    {
        Formula f = FormulaFactory.newImplication(this.visit(ctx.left), this.visit(ctx.right));
        checkNewLineHints(ctx.operator, f);
        return f;
    }
    ...
    {
        return FormulaFactory.newEquivalence(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitNegation(EvBFormulaParser.NegationContext ctx)
    {
        return FormulaFactory.newNegation(this.visit(ctx.right));
    }

    @Override
    public Formula visitDisjunction(EvBFormulaParser.DisjunctionContext ctx)
    {
        Formula f = FormulaFactory.newDisjunction(this.visit(ctx.left), this.visit(ctx.right));
        checkNewLineHints(ctx.operator, f);
        return f;
    }
    ...
    {
        return FormulaFactory.newParenthesisedPredicate(this.visit(ctx.inner));
    }

    @Override
    public Formula visitExpressionParentheses(EvBFormulaParser.ExpressionParenthesesContext ctx)
    {
        return FormulaFactory.newParenthesisedExpression(this.visit(ctx.inner));
    }

    @Override
    public Formula visitMapsToExpression(EvBFormulaParser.MapsToExpressionContext ctx)
    {
        return FormulaFactory.newMapsTo(this.visit(ctx.left), this.visit(ctx.right));
    }
    ...
    {
        return FormulaFactory.newUnaryMinus(this.visit(ctx.right));
    }

    @Override
    public Formula visitMultiplication(EvBFormulaParser.MultiplicationContext ctx)
    {
        return FormulaFactory.newMultiplication(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitDivision(EvBFormulaParser.DivisionContext ctx)
    {
        return FormulaFactory.newDivision(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitAddition(EvBFormulaParser.AdditionContext ctx)
    {
        return FormulaFactory.newAddition(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitSubtraction(EvBFormulaParser.SubtractionContext ctx)
    {
        return FormulaFactory.newSubtraction(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitModulo(EvBFormulaParser.ModuloContext ctx)
    {
        return FormulaFactory.newModulo(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitExponentiation(EvBFormulaParser.ExponentiationContext ctx)
    {
        return FormulaFactory.newExponentiation(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitMinimum(EvBFormulaParser.MinimumContext ctx)
    {
        return FormulaFactory.newMinimum(this.visit(ctx.inner));
    }

    @Override
    public Formula visitMaximum(EvBFormulaParser.MaximumContext ctx)
    {
        return FormulaFactory.newMaximum(this.visit(ctx.inner));
    }

    @Override
    public Formula visitTestBool(EvBFormulaParser.TestBoolContext ctx)
    {
        return FormulaFactory.newTestBool(this.visit(ctx.inner));
    }

    @Override
    public Formula visitCardinality(EvBFormulaParser.CardinalityContext ctx)
    {
        return FormulaFactory.newCardinality(this.visit(ctx.inner));
    }

    @Override
    public Formula visitSetMembership(EvBFormulaParser.SetMembershipContext ctx)
    {
        return FormulaFactory.newSetMembership(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitSetNotMembership(EvBFormulaParser.SetNotMembershipContext ctx)
    {
        return FormulaFactory.newSetNotMembership(this.visit(ctx.left), this.visit(ctx.right));
    }
    ...
    public Formula visitSubSet(EvBFormulaParser.SubSetContext ctx)
    {
        Formula left = this.visit(ctx.left);
        Formula right = this.visit(ctx.right);
        return FormulaFactory.newSubSet(left, right);
    }
    ...
    public Formula visitNotSubSet(EvBFormulaParser.NotSubSetContext ctx)
    {
        Formula left = this.visit(ctx.left);
        Formula right = this.visit(ctx.right);
        return FormulaFactory.newNotSubSet(left, right);
    }
    ...
    public Formula visitStrictSubSet(EvBFormulaParser.StrictSubSetContext ctx)
    {
        Formula left = this.visit(ctx.left);
        Formula right = this.visit(ctx.right);
        return FormulaFactory.newStrictSubSet(left, right);
    }
    ...
    public Formula visitNotStrictSubSet(EvBFormulaParser.NotStrictSubSetContext ctx)
    {
        Formula left = this.visit(ctx.left);
        Formula right = this.visit(ctx.right);
        return FormulaFactory.newNotStrictSubSet(left, right);
    }
    ...
    {
        return FormulaFactory.newEmptySet();
    }

    @Override
    public Formula visitIdSet(EvBFormulaParser.IdSetContext ctx)
    {
        return FormulaFactory.newIdSet();
    }

    @Override
    public Formula visitPrj1(EvBFormulaParser.Prj1Context ctx)
    {
        Formula inner_expr = this.visit(ctx.inner);
        return FormulaFactory.newPrj1(inner_expr);
    }
    ...
    public Formula visitPowerSet(EvBFormulaParser.PowerSetContext ctx)
    {
        ...
        return FormulaFactory.newPowerSet(inner_set);
    }
    ...
    {
        Formula inner_set = this.visit(ctx.inner);
        return FormulaFactory.newPower1Set(inner_set);
    }
    ...
    {
        Formula inner_set = this.visit(ctx.inner);
        return FormulaFactory.newGeneralizedUnion(inner_set);
    }
    ...
    {
        Formula inner_set = this.visit(ctx.inner);
        return FormulaFactory.newGeneralizedIntersection(inner_set);
    }
    ...
    {
        Formula inner_set = this.visit(ctx.inner);
        return FormulaFactory.newDomain(inner_set);
    }
    ...
    {
        Formula inner_set = this.visit(ctx.inner);
        return FormulaFactory.newRange(inner_set);
    }

    @Override
    public Formula visitCartesianProduct(EvBFormulaParser.CartesianProductContext ctx)
    {
        return FormulaFactory.newCartesianProduct(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitRangeSet(EvBFormulaParser.RangeSetContext ctx)
    {
        return FormulaFactory.newUpTo(this.visit(ctx.from), this.visit(ctx.to));
    }

    @Override
    public Formula visitSetUnion(EvBFormulaParser.SetUnionContext ctx)
    {
        return FormulaFactory.newSetUnion(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitSetIntersection(EvBFormulaParser.SetIntersectionContext ctx)
    {
        return FormulaFactory.newSetIntersection(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitSetMinus(EvBFormulaParser.SetMinusContext ctx)
    {
        return FormulaFactory.newSetMinus(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitRelation(EvBFormulaParser.RelationContext ctx)
    {
        return FormulaFactory.newRelation(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitTotalRelation(EvBFormulaParser.TotalRelationContext ctx)
    {
        return FormulaFactory.newTotalRelation(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitSurjectiveRelation(EvBFormulaParser.SurjectiveRelationContext ctx)
    {
        return FormulaFactory.newSurjectiveRelation(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitSurjectiveTotalRelation(EvBFormulaParser.SurjectiveTotalRelationContext ctx)
    {
        return FormulaFactory.newSurjectiveTotalRelation(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitPartialFunction(EvBFormulaParser.PartialFunctionContext ctx)
    {
        return FormulaFactory.newPartialFunction(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitTotalFunction(EvBFormulaParser.TotalFunctionContext ctx)
    {
        return FormulaFactory.newTotalFunction(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitPartialInjection(EvBFormulaParser.PartialInjectionContext ctx)
    {
        return FormulaFactory.newPartialInjection(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitTotalInjection(EvBFormulaParser.TotalInjectionContext ctx)
    {
        return FormulaFactory.newTotalInjection(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitPartialSurjection(EvBFormulaParser.PartialSurjectionContext ctx)
    {
        return FormulaFactory.newPartialSurjection(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitTotalSurjection(EvBFormulaParser.TotalSurjectionContext ctx)
    {
        return FormulaFactory.newTotalSurjection(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitTotalBijection(EvBFormulaParser.TotalBijectionContext ctx)
    {
        return FormulaFactory.newTotalBijection(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitForwardComposition(EvBFormulaParser.ForwardCompositionContext ctx)
    {
        return FormulaFactory.newForwardComposition(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitBackwardComposition(EvBFormulaParser.BackwardCompositionContext ctx)
    {
        return FormulaFactory.newBackwardComposition(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitDomainRestriction(EvBFormulaParser.DomainRestrictionContext ctx)
    {
        return FormulaFactory.newDomainRestriction(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitDomainSubtraction(EvBFormulaParser.DomainSubtractionContext ctx)
    {
        return FormulaFactory.newDomainSubtraction(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitRangeRestriction(EvBFormulaParser.RangeRestrictionContext ctx)
    {
        return FormulaFactory.newRangeRestriction(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitRangeSubtraction(EvBFormulaParser.RangeSubtractionContext ctx)
    {
        return FormulaFactory.newRangeSubtraction(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitInvertSet(EvBFormulaParser.InvertSetContext ctx)
    {
        return FormulaFactory.newInvert(this.visit(ctx.inner));
    }

    @Override
    public Formula visitRelationImage(EvBFormulaParser.RelationImageContext ctx)
    {
        return FormulaFactory.newRelationImage(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitOverride(EvBFormulaParser.OverrideContext ctx)
    {
        return FormulaFactory.newOverride(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitDirectProduct(EvBFormulaParser.DirectProductContext ctx)
    {
        return FormulaFactory.newDirectProduct(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitParallelProduct(EvBFormulaParser.ParallelProductContext ctx)
    {
        return FormulaFactory.newParallelProduct(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitOfType(EvBFormulaParser.OfTypeContext ctx)
    {
        return FormulaFactory.newOfType(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitEquals(EvBFormulaParser.EqualsContext ctx)
    {
        return FormulaFactory.newEquals(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitNotEquals(EvBFormulaParser.NotEqualsContext ctx)
    {
        return FormulaFactory.newNotEquals(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitLessThan(EvBFormulaParser.LessThanContext ctx)
    {
        return FormulaFactory.newLessThan(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitGreaterThan(EvBFormulaParser.GreaterThanContext ctx)
    {
        return FormulaFactory.newGreaterThan(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitLessThanOrEqual(EvBFormulaParser.LessThanOrEqualContext ctx)
    {
        return FormulaFactory.newLessThanOrEqual(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitGreaterThanOrEqual(EvBFormulaParser.GreaterThanOrEqualContext ctx)
    {
        return FormulaFactory.newGreaterThanOrEqual(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitUniversal(EvBFormulaParser.UniversalContext ctx)
    {
        return FormulaFactory.newUniversalQ(this.visit(ctx.left), this.visit(ctx.right));
    }

    @Override
    public Formula visitExistential(EvBFormulaParser.ExistentialContext ctx)
    {
        return FormulaFactory.newExistentialQ(this.visit(ctx.left), this.visit(ctx.right));
    }
